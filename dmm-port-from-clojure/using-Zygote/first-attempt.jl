...
function two_stroke_cycle(current_output)
    ...
    Zygote.@ignore println("===== NEW INPUT:")
    Zygote.@ignore pprintln(new_input)
    new_output = up_movement(new_input)    
    Zygote.@ignore println("***** NEW OUTPUT:")
    Zygote.@ignore pprintln(new_output)
...
function loss(state)
    ...
    Zygote.@ignore println("loss: ", 1)
    l
end

function loss2(state)
    l = state["output"]["self"]["result"]["self"]["accum"]["self"]["result"]
	l = square(l)
    Zygote.@ignore println("loss: ", 1)
    l
...
function loss3(state)
    l = 0.0
    Zygote.@ignore println("loss: ", 1)
    l
end	

function loss4(state)
    new_state = two_stroke_cycle(state["output"])
...
# here l1 contributes, but l does not contribute (this is a mistake)

function loss5(state)    
	#new_state = two_stroke_cycle(state["output"])
...
# here it is correct (and note that it counts twice, 
# via matrix, and via argument, because this is really a quadratic operation

this_grad = gradient(loss, step4)
=#

# returns (nothing,) which is not good
